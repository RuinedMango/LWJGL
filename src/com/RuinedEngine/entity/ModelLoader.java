...
import com.RuinedEngine.entity.Model.AnimatedFrame;
import com.RuinedEngine.utils.Consts;
import com.RuinedEngine.utils.Node;
...
public class ModelLoader {
	...
	private static void buildFrameMatrices(AIAnimation aiAnimation, List<Bone> boneList, Model.AnimatedFrame animatedFrame, int frame, Node node, Matrix4f parentTransformation, Matrix4f globalInverseTransform) {
		...
		for(Bone bone : affectedBones) {
			Matrix4f boneTransform = new Matrix4f(globalInverseTransform).mul(nodeGlobalTransform).mul(bone.offsetMatrix());
	...
	private static AnimMeshData processBones(AIMesh aiMesh, List<Bone> boneList) {
		...
		for(int i = 0; i < numBones; i++) {
			...
			boneList.add(bone);
			int numWieghts = aiBone.mNumWeights();
	...
	private record VertexWeight(int boneId, int vertexId, float wieght) {
	}
